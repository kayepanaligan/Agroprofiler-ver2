...
public function indexResponse(Request $request) {
    ...
    foreach ($barangays as $barangay) {
        ...
        $registeredFarmersInBarangay = $registeredFarmerIds->count();
        $unregisteredFarmersInBarangay = $unregisteredFarmerIds->count();
        $totalFarmersInBarangay = $registeredFarmersInBarangay + $unregisteredFarmersInBarangay;
        
        // Get IP eligibility
        $ipEligibility = \App\Models\Elligibility::where('name', 'like', '%ip%')
            ->orWhere('name', 'like', '%indigenous%')
            ->first();
        
        // Calculate demographics for registered farmers
        $registeredFarmersData = \App\Models\Farmer::whereIn('id', $registeredFarmerIds)->with('elligibilities')->get();
        ...
        $unregisteredFarmersData = \App\Models\Farmer::whereIn('id', $unregisteredFarmerIds)->with('elligibilities')->get();
