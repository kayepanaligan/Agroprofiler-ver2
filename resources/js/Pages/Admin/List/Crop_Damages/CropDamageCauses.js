...
const CropDamageCauses = ({ auth }) => {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get("/admin/crop-damage-causes");
    ...
    const handleSave = async () => {
        try {
            if (isEdit) {
                const response = await axios.put(`/admin/crop-damage-causes/update/${formData.id}`, formData);
                fetchData();
                ...
                const response = await axios.post("/admin/crop-damage-causes/store", formData);
                fetchData();
                toast.success(response.data.message, {
                    draggable: true,
                    closeOnClick: true,
    ...
    const handleDelete = async (id) => {
        try {
            const response = await axios.delete(`/admin/crop-damage-causes/destroy/${id}`);
            fetchData();
            toast.success(response.data.message, {
                draggable: true,
                closeOnClick: true,
    ...
    const handleMultipleDelete = async () => {
        ...
        try {
            setLoading(true);
            await axios.post("/admin/api/cropdamagecauses/delete", {
                ids: selectedIds,
            });
    ...
    return (_jsxs(AdminLayout, { user: auth.user, header: _jsx(_Fragment, { children: _jsxs("div", { className: "flex w-full justify-between", children: [_jsx("h2", { className: "text-[25px] mt-2 font-semibold text-green-600 leading-tight", children: "Crop Damage Cause Management" }), _jsxs("div", { className: "flex gap-2", children: [_jsxs(PrimaryButton, { onClick: () => handleOpen(), children: [_jsx(Plus, { size: 15 }), "Add Crop Damage Cause"] }), _jsxs(SecondaryButton, { onClick: handleMultipleDelete, disabled: selectedIds.length === 0 || loading, style: {
                            ...
                            height: "550px",
                                ...
                                border: "none",
                            } }) }), _jsx(Modal, { open: open, onClose: handleClose, children: _jsxs(Box, { className: "modal-box", sx: {
                                position: "absolute",
                                top: "50%",
                                left: "50%",
                                ...
                                p: 4,
                                borderRadius: 2,
                            }, children: [_jsx("h2", { className: "text-green-600 font-semibold text-[20px]", children: isEdit ? "Edit Cause" : "Add Cause" }), _jsx(TextField, { fullWidth: true, label: "Name", name: "name", value: formData.name, onChange: handleChange, margin: "normal", className: "green-textfield dark:text-white" }), _jsx(TextField, { fullWidth: true, label: "Description", name: "desc", value: formData.desc, onChange: handleChange, margin: "normal", className: "green-textfield dark:text-white" }), _jsx(Button, { variant: "contained", color: "primary", onClick: handleSave, style: { marginTop: 16 }, children: "Save" })] }) })] })] }));
};
