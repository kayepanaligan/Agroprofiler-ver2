...
export default function AllocationTypeList({ auth }) {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const allocationTypesResponse = await axios.get("/admin/allocation-types-list");
            ...
            const commoditiesResponse = await axios.get("/admin/commodities");
            const barangaysResponse = await axios.get("/admin/barangays");
            const cropDamagesResponse = await axios.get("/admin/crop-damages-causes");
            const eligibilitiesResponse = await axios.get("/admin/eligibilities");
            const identifierResponse = await axios.get("/admin/api/identifier");
            const fundingResponse = await axios.get("/admin/api/fundings");
    ...
    const handleEdit = (allocationId) => {
        ...
        const allocation = allocationTypes.find((item) => item.allocation_type_id === allocationId);
        if (allocation) {
    ...
    const handleDelete = async (allocationTypeId) => {
        try {
            const response = await axios.delete(`/admin/allocation/destroy/${allocationTypeId}`);
    ...
    const handleUpdateSubmit = async () => {
        ...
        try {
            const response = await axios.put(`/admin/allocationtypes/update/${formData.id}`, payload);
            fetchData();
