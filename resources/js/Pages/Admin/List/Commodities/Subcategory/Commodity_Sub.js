...
const Commodities = ({ auth }) => {
    ...
    const fetchCommodities = async () => {
        setLoading(true);
        try {
            const response = await axios.get("/admin/commodities/show");
    ...
    const fetchCategories = async () => {
        try {
            const response = await axios.get("/admin/commodity-categories-show");
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (isEdit) {
                response = await axios.post(`/admin/commodities/update/${formData.id}`, // Still using POST for update
                requestData);
            }
            else {
                response = await axios.post("/admin/commodities/store", formData);
            }
    ...
    const handleDelete = async (id) => {
        try {
            ...
            await axios.post(`/admin/commodities/destroy/${id}`, {
    ...
    const handleMultipleDelete = async () => {
        ...
        try {
            setLoading(true);
            await axios.post("/admin/api/commodities/delete", {
                ids: selectedIds,
            });
    ...
    return (_jsxs(AdminLayout, { user: auth.user, header: _jsxs("div", { className: "flex w-full justify-between", children: [_jsx("h2", { className: "text-[25px] mt-2 font-semibold text-green-600 leading-tight", children: "Commodities Management" }), _jsxs("div", { className: "flex gap-2", children: [_jsxs(PrimaryButton, { onClick: () => handleOpen(), children: [_jsx(Plus, { size: 24 }), "Add Commodity"] }), _jsxs(SecondaryButton, { onClick: handleMultipleDelete, disabled: selectedIds.length === 0 || loading, style: {
                            ...
                            height: "550px",
                                ...
                                borderRadius: 2,
                                                ...
                                                "", onChange: handleSelectChange, displayEmpty: true, children: [_jsx(MenuItem, { value: "", disabled: true, children: "Select a category" }), categories.map((category) => (_jsx(MenuItem, { value: category.id, children: category.name }, category.id)))] })] }), _jsx(PrimaryButton, { onClick: handleSave, className: "mt-4", children: "Save" })] }) })] })] }));
};
