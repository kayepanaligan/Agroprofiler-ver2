...
const IdentifierList = ({ auth }) => {
    ...
    const handleSubmit = async () => {
        if (editMode) {
            await axios.put(`/admin/update/identifier/${formData.id}`, formData);
        }
        else {
    ...
    return (_jsxs(AdminLayout, { user: auth.user, header: _jsx(_Fragment, { children: _jsxs("div", { className: "flex w-full justify-between", children: [_jsx("h2", { className: "text-[25px] mt-2 font-semibold text-green-600 leading-tight", children: "Allocation Identifier Management" }), _jsxs("div", { className: "flex gap-2", children: [_jsxs(PrimaryButton, { onClick: () => handleOpen(), children: [_jsx(Plus, { size: 24 }), "Add Allocation Identifier"] }), _jsxs(SecondaryButton, { onClick: handleMultipleDelete, disabled: selectedIds.length === 0 || loading, style: {
                            ...
                            border: "none",
                        } }), _jsx(Dialog, { open: open, onClose: handleClose, children: _jsxs("div", { className: "w-[400px] p-4 rounded-[1rem]", children: [_jsx(DialogTitle, { children: _jsx("h1", { className: "text-green-600 text-[20px] font-semibold", children: editMode
                                            ? "Edit Identifier"
                                            : "Add Identifier" }) }), _jsxs(DialogContent, { children: [_jsx(TextField, { margin: "dense", label: "Name", name: "title", fullWidth: true, value: formData.title, onChange: handleChange }), _jsx(TextField, { margin: "dense", label: "Description", name: "desc", fullWidth: true, value: formData.desc, onChange: handleChange })] }), _jsx(DialogActions, { children: _jsxs("div", { className: "px-4 flex gap-4", children: [_jsx(Button, { onClick: handleClose, children: "Cancel" }), _jsx(Button, { onClick: handleSubmit, variant: "contained", children: editMode ? "Update" : "Add" })] }) })] }) })] })] }));
};
