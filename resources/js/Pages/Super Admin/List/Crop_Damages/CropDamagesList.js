...
const CropDamages = ({ auth }) => {
    ...
    const fetchFarms = async (farmer) => {
        ...
        setLoadingFarms(true);
        try {
            const response = await axios.get(`/search/data/farms?farmer_id=${farmer.id}` // ✅ Use correct ID
            );
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            setFormData({
                ...
                proof: "",
                crop_damage_cause_id: 0,
                farm_name: "", // Provide default value
                farm_size: 0, // Provide default value
                ha: 0,
            });
            setOpenDialog(false);
        }
    ...
    return (_jsxs(Authenticated, { user: auth.user, header: _jsx(_Fragment, { children: _jsxs("div", { className: "flex w-full justify-between", children: [_jsx("h2", { className: "text-[24px] mt-2 font-semibold text-green-600 leading-tight", children: "Crop Damages Management" }), _jsxs("div", { className: "flex gap-4", children: [_jsxs(PrimaryButton, { onClick: () => setOpenDialog(true), children: [_jsx(Plus, { size: 24 }), "Add Data"] }), _jsxs(SecondaryButton, { onClick: handleMultipleDelete, disabled: selectedIds.length === 0 || loading, style: {
                            ...
                            height: "520px",
                                ...
                                border: "none",
                                                ...
                                                display: "flex",
                                                borderRadius: "15px",
                                            } }) })), _jsx("br", {}), _jsx("br", {}), _jsx(Autocomplete, { options: farmers, getOptionLabel: (option) => option.label ?? "", loading: loadingFarmers, onInputChange: (event, newInputValue) => setSearchQuery(newInputValue), onChange: (event, newValue) => {
