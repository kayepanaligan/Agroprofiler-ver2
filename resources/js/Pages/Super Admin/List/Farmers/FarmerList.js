...
export default function FarmerList({ auth, farmers, barangays = [], }) {
    ...
    const handleInputChange = async (e) => {
        ...
        if (name === "rsbsa_ref_no" && value.length >= 5) {
            try {
                const response = await axios.get(`/api/check-farmer?rsbsa_ref_no=${value}`);
        ...
        if (name === "firstname" || name === "lastname") {
            try {
                ...
                if (firstname.trim() !== "" && lastname.trim() !== "") {
                    const response = await axios.get(`/api/check-farmer?firstname=${firstname}&lastname=${lastname}`);
