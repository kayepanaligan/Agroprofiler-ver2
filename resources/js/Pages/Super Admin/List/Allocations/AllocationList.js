...
export default function AllocationList({ auth, allocation = {
    ...
    const fetchRemainingBalance = async (allocationTypeId) => {
        ...
        try {
            const response = await axios.get(`/api/allocations/remaining-balance/${allocationTypeId}`);
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            ...
            const response = await axios.put(`/allocations/update/${selectedAllocation.id}`, updates, { headers: { "X-CSRF-TOKEN": csrfToken } });
    ...
    useEffect(() => {
        if (selectedAllocation?.allocation_type_id) {
            axios
                .get(`/api/allocation-type/${selectedAllocation.allocation_type_id}/dependencies`)
                .then((res) => {
    ...
    useEffect(() => {
        if (newAllocation.allocation_type_id) {
            axios
                .get(`/api/allocation-type/${newAllocation.allocation_type_id}/dependencies`)
                .then((res) => {
                setFilteredBarangays(res.data.barangays);
                setFilteredCommodities(res.data.commodities);
                setNewAllocation((prev) => ({
    ...
    return (_jsxs(Authenticated, { user: auth.user, header: _jsx(_Fragment, { children: _jsxs("div", { className: "flex w-full justify-between", children: [_jsx("h2", { className: "text-[25px] mt-2 font-semibold text-green-600 leading-tight", children: "Allocation Management" }), _jsxs("div", { className: "flex gap-4", children: [_jsxs(PrimaryButton, { onClick: openModal, children: [_jsx(Plus, { size: 24 }), "Add Allocation"] }), _jsxs(SecondaryButton, { onClick: handleMultipleDelete, disabled: selectedIds.length === 0 || loading, style: {
                                    ...
                                    }, className: "mb-3 w-full rounded-lg border-gray-300 dark:text-white dark:bg-[#122231]", children: [_jsx("option", { className: "dark:text-white dark:bg-[#122231]", value: "", children: "Allocation Type" }), allocationType.map((allocationType) => (_jsx("option", { className: "dark:text-white dark:bg-[#122231]", value: allocationType.value, children: allocationType.label }, allocationType.value)))] }), _jsx(Select, { options: farmerss, isLoading: loadingFarmers, value: newAllocation.farmer_id
                        ...
                        border: "none",
                                        ...
                                        ...prev,
                                        commodity_id: Number(e.target.value),
                                    })), className: "mt-3 w-full rounded-lg border-gray-300 dark:text-white dark:bg-[#122231]", children: [_jsx("option", { className: "dark:text-white dark:bg-[#122231]", value: "", children: "Select Commodity" }), filteredCommodities.map((commodity) => (_jsx("option", { className: "dark:text-white dark:bg-[#122231]", value: commodity.id, children: commodity.name }, commodity.id)))] }), _jsx(TextInput, { name: "funding_id", value: selectedAllocation.funding_name || "", className: "mt-3 w-full rounded-lg border-gray-300", disabled: true }), _jsx(TextInput, { type: "date", name: "date_received", value: selectedAllocation.date_received instanceof
                                        Date
