...
export default function Sidebar({ user }: Props) {
    ...
    return (
        <div
            className={`fixed index-20 w-[16%] mt-5 p-5 overflow-hidden bg-white rounded-[1rem] ml-3 shadow  {expanded ? "w-[10rem]" : "w-[20rem]}`}
        >
            <div className="w-full h-auto p-4 flex justify-center content-center">
                <button
                    onClick={() => setExpanded((curr) => !curr)}
                    className="transition-all font-semibold text-green-900 text-[18px]"
                >
                    <img
            ...
            <div className="border-t">
                ...
                <div className="flex gap-2 justify-center content-center">
                    <Link href={route("logout")} method="post" as="button">
                        <span className="inline-flex mt-3 gap-1 text-[10px] font-semibold text-red-600 border p-2 rounded-full shadow-lg  hover:bg-red-200 hover:text-red-400">
                            <Power size={15} />
                            Log Out
                        </span>
                    </Link>
