...
interface HeatmapProps {
    heatmapData: Record<string, any>; // Assuming the structure of the heatmap data passed from backend
...
const Heatmap: React.FC<HeatmapProps> = ({
    ...
    const [farmers, setFarmers] = useState<string[]>(["All"]);
    const [highValueCrops, setHighValueCrops] = useState<string[]>(["All"]);
    const [subtypes, setSubtypes] = useState<string[]>(["All"]);
    ...
    useEffect(() => {
        ...
        axios
            ...
            .then((response) => {
                const types = response.data.map((type: any) => type.name);
                setAllocations(["All", ...types]);
            })
            .catch((error) =>
        ...
        axios
            .get("/commodity-categories-show")
            .then((response) => {
                const categories: Record<string, string[]> = {};
                const allCommodities: string[] = ["All"];

                response.data.forEach((category: any) => {
                    categories[category.name] = category.commodities.map(
                        (c: any) => c.name
                    );
                    allCommodities.push(...categories[category.name]);
                });
        ...
        // Fetch Farmers Distribution
        axios
            .get("/api/farmers-types")
            .then((response) => {
                const farmerTypes = response.data.map((type: any) => type.name);
                setFarmers(["All", ...farmerTypes]);
            })
            .catch((error) => console.error("Error fetching farmers:", error));

        axios
            .get("/api/high-value-crops")
            .then((response) => {
                const crops = response.data.map((crop: any) => crop.name);
                setHighValueCrops(["All", ...crops]);
            })
            .catch((error) =>
                console.error("Error fetching high-value crops:", error)
            );
    }, []);

    useEffect(() => {
        // Update subtypes when category changes
        if (view === "commodities_categories") {
            setSubtypes(["All", ...(commodityCategories[category] || [])]);
        }
    }, [category, view, commodityCategories]);
