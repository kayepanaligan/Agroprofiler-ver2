...
const Heatmap: React.FC<HeatmapProps> = ({
    ...
    useEffect(() => {
        if (view === "allocations" && allocationType.length > 0) {
            ...
            setSubtype(categories[category][0] || "All");
        } else if (view === "farmers") {
            // Default view for farmers
            setSubtype("registered");
        } else if (view === "commodityCategories" && categories.length > 0) {
            // Default view for commodity categories, setting the first category's first subtype as default
            setCategory(categories[0]?.name || "All");
            setSubtype(categories[0]?.subtypes[0] || "All");
        }
