...
const DistributionSelector = () => {
    ...
    useEffect(() => {
        ...
        axios.get("/api/allocation-types")
            .then(response => {
                const allocationTypes = response.data.map((type: any) => type.name);
                setDistributions(prev => ({ ...prev, allocations: ["All", ...allocationTypes] }));
            })
            .catch(error => console.error("Error fetching allocations:", error));

        // Fetch commodity categories with their subtypes
        axios.get("/api/commodity-categories")
            .then(response => {
                const categoriesData = response.data.reduce((acc: any, category: any) => {
                    acc[category.name] = category.commodities.map((c: any) => c.name);
                    return acc;
                }, {});
                setDistributions(prev => ({ ...prev, commodities_categories: categoriesData }));
            })
            .catch(error => console.error("Error fetching commodity categories:", error));
    }, []);
    ...
    const handleChangeSubtype = (event: React.ChangeEvent<HTMLSelectElement>) => {
