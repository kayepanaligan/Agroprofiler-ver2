...
const GroupedBarChart: React.FC<GroupedBarChartProps> = ({
    ...
    const chartData = Object.keys(heatmapData).map((barangay) => {
        const entry = heatmapData[barangay] ?? {};
        console.log(`Processing barangay: ${barangay}`, entry);

        const rowData: any = { name: barangay }; // Initialize row with barangay name
        ...
        // Process commodity categories dynamically
        if (
        ...
        // Process farmer distribution
        if (distributionType === "farmers" && entry.farmers) {
            rowData["Registered"] = entry.farmers["Registered"] || 0;
            rowData["Unregistered"] = entry.farmers["Unregistered"] || 0;
        }

        console.log("Processed rowData:", rowData);
        return rowData;
    });
