...
const Heatmap: React.FC<HeatmapProps> = ({
    ...
    const getColor = (barangayName: string) => {
        ...
        if (view === "allocations" && subtype !== "All") {
            const allocationData =
                heatmapData[barangayName]?.allocations?.[subtype];

            if (allocationData) {
                const allocationAmount = parseFloat(allocationData.amount) || 0;
                const totalAllocationType = allocationType.find(
                    (type) => type.name === subtype
                );
                const totalAllocationAmount = totalAllocationType
                    ? parseFloat(totalAllocationType.totalAmount) || 0
                    : 0;
                console.log("totalAllocationType:", totalAllocationAmount);
                if (totalAllocationAmount > 0) {
                    percentage =
                        (allocationAmount / totalAllocationAmount) * 100;
                }
            }
    ...
    const onEachFeature = (feature: any, layer: any) => {
        ...
        ${barangayName} received ${allocationPercentage}% of ${subtype} (${allocationAmount})
    `;
        } else if (
            view === "commodities" &&
            category !== "All" &&
