...
const Heatmap: React.FC<HeatmapProps> = ({
    ...
    const distributions: Record<string, string[]> = {
        ...
        commodities: commodities, // Keep "farmers" as the current list of farmers
        farmers: farmers,
    };
    ...
    const calculatePercentageIntensity = (barangayName: string) => {
        if (view === "allocations" && subtype !== "All") {
            const barangayTotal =
                heatmapData[barangayName]?.allocations?.[subtype] || 0;
            const overallTotal = Object.values(heatmapData)
                .map((data: any) => data.allocations?.[subtype] || 0)
                .reduce((acc, val) => acc + val, 0);

            return overallTotal > 0 ? (barangayTotal / overallTotal) * 100 : 0;
        }

        if (view === "farmers" && subtype !== "All") {
            const barangayTotal =
                heatmapData[barangayName]?.farmers?.[subtype] || 0;
            const totalFarmers = Object.values(
                heatmapData[barangayName]?.farmers || {}
            ).reduce((acc, val) => acc + val, 0);

            return totalFarmers > 0 ? (barangayTotal / totalFarmers) * 100 : 0;
        }
