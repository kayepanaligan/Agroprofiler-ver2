...
const Heatmap: React.FC<HeatmapProps> = ({
    ...
    useEffect(() => {
        if (view === "allocations") {
            // Set allocation types as subtypes
            ...
            // Default to showing all commodities
    ...
    const handleChangeView = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        setSubtype(""); // Reset subtype when switching views
        setCategory("All"); // Reset category when switching views
    };
    ...
    return (
        <div className="heatmap-container">
            <div className="select-container mb-10 grid lg:grid-flow-row lg:grid-rows-1 md:grid-flow-col md:grid-cols-2 gap-4">
                ...
                {view === "commodities" && (
                    <select
                        id="commodity-type-select"
                        value={subtype}
                        onChange={handleChangeSubtype}
                        className="rounded-[12px] border-slate-500 cursor-pointer focus:border-green-500 sm:text-[14px]"
                    >
                        <option value="all">All Commodities</option>
                        {commoditiesCategories.map((commodity) => (
                ...
                {view === "commodities_categories" && (
                    <>
                        ...
                        {/* Subtype Selection if a Specific Category is Chosen */}
                        {category !== "All" && (
                            <select
                                value={subtype}
                                onChange={handleChangeSubtype}
