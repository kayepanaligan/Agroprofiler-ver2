...
import { Download, Pencil, Plus, PlusIcon, Trash, Trash2 } from "lucide-react";
...
export default function AllocationList({
    ...
    const fetchRemainingBalance = async (allocationTypeId: any) => {
        ...
        try {
            const response = await axios.get(
                `/admin/api/allocations/remaining-balance/${allocationTypeId}`
            );
    ...
    const handleEdit = (allocationID: number) => {
        ...
        if (allocation) {
            setSelectedAllocation({
                ...allocation,
                farmer_id: allocation.farmer?.id || "",
                commodity_id: allocation.commodity?.id || "",
                brgy_id: allocation.barangay?.id || "",
            });
    ...
    const handleDelete = async (allocations: Allocation) => {
        ...
        if (
            ...
            try {
                await router.delete(
                    `/admin/allocations/destroy/${allocations}`
                );
    ...
    const handleUpdate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await axios.put(
                `/admin/allocations/update/${selectedAllocation.id}`,
                updates,
    ...
    useEffect(() => {
        if (selectedAllocation?.allocation_type_id) {
            axios
                .get(
                    `/admin/api/allocation-type/${selectedAllocation.allocation_type_id}/dependencies`
                )
                .then((res) => {
            ...
            setSelectedAllocation((prev) =>
                prev ? { ...prev, identifier_id: 0, funding_id: 0 } : prev
            );
        }
    ...
    useEffect(() => {
        if (newAllocation.allocation_type_id) {
            axios
                .get(
                    `/admin/api/allocation-type/${newAllocation.allocation_type_id}/dependencies`
                )
                .then((res) => {
                    setFilteredBarangays(res.data.barangays);
                    setFilteredCommodities(res.data.commodities);
                    ...
                    <div className="p-10">
                        ...
                        <form onSubmit={handleUpdate}>
                            ...
                            <select
                                ...
                                className="mt-3 w-full rounded-lg dark:text-white dark:bg-[#122231] border-gray-300"
                            >
                                <option
                                    className="dark:text-white dark:bg-[#122231]"
                                    value=""
