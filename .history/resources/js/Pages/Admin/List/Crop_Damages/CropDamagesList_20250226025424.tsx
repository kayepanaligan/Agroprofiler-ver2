...
const customStyles = (isDarkMode: boolean) => ({
    ...
    dropdownIndicator: (base: any) => ({
        ...base,
        color: isDarkMode ? "white" : "black", // Ensures dropdown arrow is visible
    }),
...
const CropDamages = ({ auth }: PageProps) => {
    ...
    const handleFarmerChange = async (selectedOption: any) => {
        ...
        try {
            const response = await axios.get(
                `/admin/farms/${selectedOption.value}`
            );
            setFarms(
    ...
    const handleSelectChange = (
        ...
        if (field === "farm_id" && selectedOption) {
            ...
            let farmSize = selectedFarm?.farm_size ?? 0; // ✅ Ensure farm_size is always a number
    ...
    const fetchFarms = async (farmer: Farmer) => {
        if (!farmer || !farmer.id) {
            setFarms([]);
            return;
        }
        ...
        try {
            const response = await axios.get(
                `/admin/search/data/farms?farmer_id=${farmer.id}` // ✅ Use correct ID
            );
            const farmOptions = response.data.map((farm: any) => ({
                value: farm.value,
                label: `${farm.name} (${farm.ha} ha)`,
            }));
    ...
    const handleFarmChange = async (
        ...
        try {
            const response = await axios.get(
                `/admin/farms/details/${selectedFarmId}`
            );
