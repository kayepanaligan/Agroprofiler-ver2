...
export default function Dashboard({
    ...
    // Load saved color from backend or localStorage
    useEffect(() => {
        const fetchColor = async () => {
            try {
                const response = await axios.get("/api/user/color");
                if (response.data.color) {
                    setSelectedColor(response.data.color);
                }
            } catch (error) {
                console.error("Error fetching saved color:", error);
    ...
    // Save the color when changed
    const handleColorChange = async (
        event: React.ChangeEvent<HTMLInputElement>
    ) => {
        const newColor = event.target.value;
        setSelectedColor(newColor);
        localStorage.setItem("userColor", newColor); // Save locally

        // Optionally save to backend
        try {
            await axios.post("/api/user/color", { color: newColor });
        } catch (error) {
    ...
    return (
        ...
        >
            ...
            <div className="">
                ...
                <div className="mb-4 grid lg:grid-cols-2 md:grid-cols-1 gap-4">
