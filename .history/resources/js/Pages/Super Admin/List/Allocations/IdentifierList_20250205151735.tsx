...
const IdentifierList = ({ auth }: PageProps) => {
    const [identifer, setidentifer] = useState<Funding[]>([]);
    ...
    const fetchidentifer = async () => {
        const response = await axios.get("http://127.0.0.1:8000/api/identifer");
    ...
    const handleSubmit = async () => {
        if (editMode) {
            await axios.put(`/update/identifer/${formData.id}`, formData);
        } else {
            await axios.post("/store/identifer", formData);
        }
        fetchidentifer();
        handleClose();
    };
    ...
    const handleDelete = async (id: number) => {
        await axios.delete(`/destroy/identifer/${id}`);
        fetchidentifer();
    };
