    ...
    const fetchFarms = async (farmer: Farmer) => {
        ...
        try {
            const response = await axios.get(
                `/search/data/farms?farmer_id=${farmer.id}` // ✅ Use correct ID
            );
    ...
    return (
        ...
        >
            ...
            <div>
                ...
                >
                    <DataGrid
                        rows={(cropDamages || []) // Ensure it's not undefined
                            .filter((row) => row && row.id !== undefined) // Remove undefined or missing `id`
                            .map((row, index) => ({
                                ...row,
                                id: row.id || index, // Assign a fallback id if missing
                            }))}
                        columns={columns}
                        initialState={{
                            pagination: {
