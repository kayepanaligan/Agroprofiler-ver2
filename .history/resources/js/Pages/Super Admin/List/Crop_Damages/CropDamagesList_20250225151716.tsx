...
const CropDamages = ({ auth }: PageProps) => {
        ...
        setFormData((prevData) => ({
            ...
            severity:
                name === "total_damaged_area"
                    ? calculateSeverity(newValue, prevData.farm_size) // ✅ Uses farm_size
                    : prevData.severity,
        }));
    };
    ...
    useEffect(() => {
        if (formData.farm_id && formData.total_damaged_area) {
            ...
            if (selectedFarm) {
                const farmSize = parseFloat(selectedFarm.ha);
                console.log("🛠️ Selected Farm Size:", farmSize);
                console.log(
                    "🛠️ Total Damaged Area:",
