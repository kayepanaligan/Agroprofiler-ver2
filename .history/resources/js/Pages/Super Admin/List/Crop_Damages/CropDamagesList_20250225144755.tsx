...
const CropDamages = ({ auth }: PageProps) => {
    ...
    const handleSelectChange = (newValue) => {
        if (newValue) {
            setSelectedFarm(newValue); // Store selected farm
            setFormData((prev) => ({
                ...prev,
                farm_id: newValue.value, // Store farm ID
                severity: "", // Reset severity when farm changes
            }));
        } else {
            setSelectedFarm(null);
            setFormData((prev) => ({ ...prev, farm_id: "", severity: "" }));
        }
    };
    ...
    const handleFormChange = (event) => {
        ...
        if (name === "total_damaged_area" && selectedFarm) {
            updateSeverity(value, selectedFarm.hectares); // Calculate severity
        }
    };

    const updateSeverity = (damagedArea, farmSize) => {
        const damagePercentage =
            (parseFloat(damagedArea) / parseFloat(farmSize)) * 100;
        let severityLevel = "";

        if (damagePercentage <= 20) severityLevel = "Low";
        else if (damagePercentage > 20 && damagePercentage <= 40)
            severityLevel = "Medium";
        else if (damagePercentage > 40) severityLevel = "High";

        setFormData((prev) => ({ ...prev, severity: severityLevel }));
    };
        ...
        try {
            ...
            setFarms(
                response.data.map((farm) => ({
                    value: farm.value,
                    label: `${farm.name} (${farm.ha} ha)`, // Show name + hectares
                }))
            );
        } catch (error) {
            console.error("Error fetching farms", error);
