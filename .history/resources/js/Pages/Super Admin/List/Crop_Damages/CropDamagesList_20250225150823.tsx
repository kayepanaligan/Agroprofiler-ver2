...
const CropDamages = ({ auth }: PageProps) => {
        ...
        setFormData((prevData) => ({
            ...
            severity:
                name === "total_damaged_area"
                    ? calculateSeverity(newValue, prevData.farm_size) // Use latest farm_size
                    : prevData.severity,
        }));
    };
    ...
    const handleSelectChange = (selectedOption: any, field: string) => {
        if (field === "farm_id" && selectedOption) {
            ...
            const selectedFarm = farms.find(
                (farm) => farm.value === selectedOption.value
            );

            setFormData((prevData) => {
                const newFarmSize = selectedFarm
                    ? parseFloat(selectedFarm.hectares)
                    : 0;
                return {
                    ...prevData,
                    farm_id: selectedOption.value,
                    farm_size: newFarmSize, // ✅ Store hectares properly
                    severity: calculateSeverity(
                        prevData.total_damaged_area,
                        newFarmSize
                    ), // ✅ Ensure severity updates correctly
                };
            });
        } else {
            setFormData((prevData) => ({
        ...
        try {
            ...
            setFarms(
                response.data.map((farm) => ({
                    ...
                    hectares: farm.ha, // ✅ Ensure hectares is stored
                }))
            );
        } catch (error) {
            console.error("Error fetching farms", error);
    ...
    useEffect(() => {
        if (formData.farm_size > 0 && formData.total_damaged_area > 0) {
            setFormData((prevData) => ({
                ...prevData,
                severity: calculateSeverity(
                    prevData.total_damaged_area,
    ...
    }, [formData.farm_size, formData.total_damaged_area]);
    // Runs when farm or damaged area changes
