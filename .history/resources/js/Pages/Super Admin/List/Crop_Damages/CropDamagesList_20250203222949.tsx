...
const CropDamages = ({ auth }: PageProps) => {
    ...
    const handleFarmChange = async (event) => {
        ...
        try {
            const response = await axios.get(`/farm/${selectedFarmId}`); // Fetch the selected farm details
            const farmSize = response.data.ha; // Get farm size in hectares
            setFormData((prev) => ({
                ...prev,
                farm_size: farmSize, // Store the farm size in state
            }));
        } catch (error) {
    ...
    const handleFormChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (event.target.name === "total_damaged_area") {
    ...
    const calculateSeverity = (totalDamagedArea: number, farmSize: number) => {
        if (!farmSize || farmSize <= 0) return ""; // Avoid division by zero

        const damagePercentage = (totalDamagedArea / farmSize) * 100; // Convert to percentage
    ...
    const handleFormChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Calculate severity only if farm_id and farm size are selected
        if (formData.farm_id && formData.farm_size) {
            updatedFormData.severity = calculateSeverity(
                Number(updatedFormData.total_damaged_area),
                formData.farm_size
            );
        }
    ...
    const calculateSeverity = (totalDamagedArea: number, farmSize: number) => {
        if (!farmSize || farmSize === 0) return ""; // Prevent division by zero
