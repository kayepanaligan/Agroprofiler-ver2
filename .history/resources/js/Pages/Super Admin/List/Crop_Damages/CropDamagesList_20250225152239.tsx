...
const CropDamages = ({ auth }: PageProps) => {
        ...
        setFormData((prevData) => {
            ...
            const newSeverity =
                name === "total_damaged_area"
                    ? calculateSeverity(newValue, prevData.farm_size)
                    : prevData.severity;
    ...
    const handleSelectChange = (selectedOption: any, field: string) => {
        if (field === "farm_id" && selectedOption) {
            ...
            // Extract the farm size from the label (e.g., "Kaye (34.00 ha)")
            let farmSize = 0;
            if (selectedFarm) {
                const match = selectedFarm.label.match(/\(([\d.]+)\s*ha\)/); // Extracts 34.00 from "Kaye (34.00 ha)"
