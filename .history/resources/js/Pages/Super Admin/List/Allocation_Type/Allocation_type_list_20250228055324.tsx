...
export default function AllocationTypeList({ auth }: PageProps) {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const fundingResponse = await axios.get<Funding[]>("/api/fundings");
    ...
    const rows: AllocationTypeRow[] = (allocationTypes || []).map(
        (type, index) => ({
            ...
            amount: type.amount,
            funding_name: type.funding_name || "No Funding ID",
            identifier_title: type.identifier_title || "No Identifier",

            // Ensure identifier_id and funding_id exist
            identifier_id: type.identifier_id || "No Identifier ID",
            funding_id: type.funding_id || "No Funding ID",

            // Ensure allocation_type_id exists
            allocation_type_id:
                type.allocation_type_id || "No Allocation Type ID",

            // Ensure these fields remain as arrays instead of strings
            allocation_type_commodities: Array.isArray(
                type.allocation_type_commodities
            )
                ? type.allocation_type_commodities.map(
                      (c: { commodity_name: string }) => c.commodity_name
                  )
                : [],
            allocation_type_barangays: Array.isArray(
                type.allocation_type_barangays
            )
                ? type.allocation_type_barangays.map(
            ...
            allocation_type_crop_damage_causes: Array.isArray(
                ...
                ? type.allocation_type_crop_damage_causes.map(
            ...
            allocation_type_elligibilities: Array.isArray(
                ...
                ? type.allocation_type_elligibilities.map(
                      (e: { elligibility_type: string }) => e.elligibility_type
                  )
                : [],
        })
    );
    ...
    const handleEdit = (allocationId: number) => {
        ...
        if (allocation) {
            ...
            setFormData({
                id: allocation.allocation_type_id,
                name: allocation.name || "",
                desc: allocation.desc || "",
                amount: allocation.amount,
                funding_id: allocation.funding_id,
                identifier_id: allocation.identifier_id,
                commodityIds:
