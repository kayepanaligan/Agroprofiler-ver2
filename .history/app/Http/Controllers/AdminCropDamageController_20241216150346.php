...
class AdminCropDamagesController extends Controller
{
...
public function store(Request $request)
{
    try {
        ...
        if ($request->hasFile('proof')) {
            ...
            $file->storeAs('public/proof', $fileName); // Save to storage/app/public/proof
            $proofPath = '/storage/proof/' . $fileName; // Correct path for URL generation
        }
        ...
        DB::insert(
            ...
            (farmer_id, commodity_id, brgy_id, total_damaged_area, partially_damaged_area, crop_damage_cause_id, area_affected, severity, remarks, proof, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [
                $request->farmer_id,
