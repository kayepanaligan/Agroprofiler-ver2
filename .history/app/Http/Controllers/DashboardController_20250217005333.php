...
class DashboardController extends Controller
{
  ...
  public function index(Request $request) {
    ...
    // Fetch allocations with or without year filtering
    $allocationsQuery = Allocation::query();
    if ($selectedYear !== 'all') {
        $allocationsQuery->whereYear('created_at', $selectedYear);
    }
    $allocations = $allocationsQuery->get();

    // Clone the base query for farmers before modifying it
    $baseFarmersQuery = Farmer::query();
    if ($selectedYear !== 'all') {
        $baseFarmersQuery->whereYear('created_at', $selectedYear);
    }

    // Separate queries to ensure correct results
    $registeredFarmers = (clone $baseFarmersQuery)->where('status', 'registered')->count();
    $unregisteredFarmers = (clone $baseFarmersQuery)->where('status', 'unregistered')->count();
    $totalFarmers = $baseFarmersQuery->count(); // This remains unchanged
...
public function indexResponse(Request $request) {
    ...
    $years = Allocation::selectRaw('YEAR(created_at) as year')
        ->union(Farmer::selectRaw('YEAR(created_at) as year'))
        ->union(Farm::selectRaw('YEAR(created_at) as year'))
        ->distinct()
        ->pluck('year');

    $allocationsQuery = Allocation::whereYear('created_at', $selectedYear);
        if ($selectedMonth) {
            $allocationsQuery->whereMonth('created_at', $selectedMonth);
        }
    $allocations = $allocationsQuery->get();

    $farmersQuery = Farmer::whereYear('created_at', $selectedYear);
        if ($selectedMonth) {
            $farmersQuery->whereMonth('created_at', $selectedMonth);
        }
    $registeredFarmers = $farmersQuery->where('status', 'registered')->count();
    $unregisteredFarmers = $farmersQuery->where('status', 'unregistered')->count();
    $totalFarmers = $farmersQuery->count();

    $farmsQuery = Farm::whereYear('created_at', $selectedYear);
    if ($selectedMonth) {
        $farmsQuery->whereMonth('created_at', $selectedMonth);
    }
    ...
    foreach ($barangays as $barangay) {
        $barangayFarms = $farms->where('brgy_id', $barangay->id);
        ...
        foreach ($commodityCategories as $category) {
            ...
            foreach ($category->commodities as $commodity) {
                $count = $barangayFarms->where('commodity_id', $commodity->id)->count();
                $categoryCommodities[$commodity->name] = $count;
            }
