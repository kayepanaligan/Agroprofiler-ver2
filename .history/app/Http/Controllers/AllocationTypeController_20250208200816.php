   ...
   public function index()
{
    $allocationTypes = AllocationType::with([
        ...
        'cropDamageCauses',
        'elligibilities',
        'identifier',  // Ensure these relationships exist
        'funding'
    ])->get();
    ...
    $output = $allocationTypes->map(function ($allocationType) {
        return [
            ...
            'identifier_id' => optional($allocationType->identifier)->id, // Ensure the relation exists
            'funding_id' => optional($allocationType->funding)->id, // Ensure the relation exists
            'barangays' => $allocationType->barangays->map(fn($b) => ['id' => $b->id, 'name' => $b->name]),
            'commodities' => $allocationType->commodities->map(fn($c) => ['id' => $c->id, 'name' => $c->name]),
            'crop_damage_causes' => $allocationType->cropDamageCauses->map(fn($cd) => ['id' => $cd->id, 'name' => $cd->name]),
            'elligibilities' => $allocationType->elligibilities->map(fn($e) => ['id' => $e->id, 'elligibility_type' => $e->elligibility_type])
        ];
    });
   ...
   public function show()
    ...
    public function store(Request $request)
    {
        ...
        $allocationType = AllocationType::create([
            ...
            'funding_id' => $validated['funding_id'],
