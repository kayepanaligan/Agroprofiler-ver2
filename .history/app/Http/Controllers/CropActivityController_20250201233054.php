...
{
    \Log::info('Incoming Update Request:', $request->all()); // Log incoming request data
    ...
    if ($request->hasFile('file_path')) {
        ...
        if ($cropActivity->file_path && Storage::exists('public/' . $cropActivity->file_path)) {
            Storage::delete('public/' . $cropActivity->file_path);
        }

        // Store the new file and generate the relative path
        $filePath = $request->file('file_path')->store('crop_activities', 'public');
        // Generate the full URL for the file path
        $filePath = Storage::url($filePath);
    } else {
        // If no new file is uploaded, keep the old file path
        $filePath = $cropActivity->file_path;
    }

    // Update the crop activity using DB Query Builder
    DB::table('crop_activities')
        ->where('id', $cropActivity->id)
        ->update([
            'title' => $request->title,
            'desc' => $request->desc,
            'file_path' => $filePath,  // Updated file path (or existing one)
            'date' => $request->date,
            'updated_at' => now(),
        ]);
