  ...
  public function update(Request $request, CropActivity $cropActivity)
{
    ...
    $filePath = $request->file('file_path')->store('crop_activities', 'public');
    $validated['file_path'] = '/storage/' . $filePath; // Save public URL
    } else {
        $validated['file_path'] = $cropActivity->file_path; // Keep existing file
    }
// Update the crop activity

    // Update the record using Eloquent
    $cropActivity->update($validated);
